...
export function ProviderHistoryDialog({ providerId, providerName, isOpen, onOpenChange }: ProviderHistoryDialogProps) {
  const { orgId } = useAuth();
  const [history, setHistory] = useState<{ tasks: TaskWithDetails[], expenses: Expense[] } | null>(null);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (isOpen && providerId && orgId) {
      setIsLoading(true);
      setHistory(null);
      Promise.all([
        getTasksByProviderId(providerId, orgId),
        getPropertyExpensesByProviderId(providerId, orgId)
      ]).then(([tasks, expenses]) => {
      ...
      <DialogContent className="sm:max-w-4xl">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
                             ...
                             <TableBody>
                                {history.tasks.map(task => (
                                    <TableRow key={task.id}>
                                        ...
                                        <TableCell className="text-right">{formatCurrency(task.estimatedCost, task.costCurrency || 'ARS')}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
            ...
            <p className="text-center py-12 text-muted-foreground italic">No se encontró historial para este proveedor.</p>
        )}
