...
export default function ReportsClient({ financialSummary, tenantsByOrigin, expensesByCategory, expensesByProperty, bookingsByOrigin, bookingStatus }: ReportsClientProps) {
  ...
  const hasArsData = financialSummary.ars.some(s => 
    s.totalIncome !== 0 || s.totalPayments !== 0 || s.balance !== 0 || s.totalExpenses !== 0 || s.netResult !== 0
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {hasUsdData && (
         <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {financialSummary.usd.filter(s => s.totalIncome !== 0 || s.totalExpenses !== 0).map(item => (
      ...
      {hasArsData && (
        <>
             <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {financialSummary.ars.filter(s => s.totalIncome !== 0 || s.totalExpenses !== 0).map(item => (
