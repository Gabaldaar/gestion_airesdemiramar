...
import { addProviderCategory, updateProviderCategory, deleteProviderCategory, seedProviderCategoriesAction } from '@/lib/actions';
...
function CategoryAddRow({ onActionComplete }: { onActionComplete: () => void }) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
        const result = await addProviderCategory(initialState, formData);
        setState(result);
    });
  }
...
function CategoryEditRow({ category, onCancel, onUpdated }: { category: ProviderCategory, onCancel: () => void, onUpdated: () => void }) {
    const [state, setState] = useState(initialState);
    const [isPending, startTransition] = useTransition();
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const formData = new FormData(event.currentTarget);
        startTransition(async () => {
            const result = await updateProviderCategory(initialState, formData);
            setState(result);
        });
    }
...
function CategoryDeleteAction({ categoryId, onDeleted }: { categoryId: string, onDeleted: () => void }) {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const formData = new FormData(event.currentTarget);
        startTransition(async () => {
            const result = await deleteProviderCategory(initialState, formData);
            setState(result);
        });
    }
        ...
        <AlertDialog>
            ...
            <AlertDialogContent>
                 <form onSubmit={handleSubmit}>
                    ...
                    <AlertDialogDescription>
                        {t('settings.categories.providers_delete_confirm')}
                    </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
...
export default function ProviderCategoryManager({ initialCategories, onCategoriesChanged }: { initialCategories: ProviderCategory[], onCategoriesChanged: () => void }) {
  const { t } = useTranslation();
  ...
  const handleLoadExamples = () => {
      ...
      startSeedTransition(async () => {
          const result = await seedProviderCategoriesAction(orgId);
          if (result.success) {
                ...
                <Card key={category.id}>
                    <CardContent className="p-2">
                        <form onSubmit={(e) => {
                            ...
                            updateProviderCategory(initialState, formData).then(refreshCategories);
