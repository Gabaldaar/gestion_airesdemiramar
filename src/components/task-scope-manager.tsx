...
function ScopeAddRow({ onActionComplete }: { onActionComplete: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} ref={formRef} className="flex items-center gap-2 p-2 border-t">
        ...
        <Input name="name" placeholder={t('settings.scopes.new_placeholder')} className="flex-grow" required />
        ...
        <AlertDialog>
            ...
            <AlertDialogContent>
                 <form onSubmit={handleSubmit}>
                    ...
                    <AlertDialogDescription>
                        {t('settings.scopes.delete_confirm')}
                    </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                    ...
                    <AlertDialogAction asChild>
                        <Button type="submit" variant="destructive" disabled={isPending}>
                             {isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : t('common.confirm_delete.confirm')}
                        </Button>
                    </AlertDialogAction>
                    </AlertDialogFooter>
                </form>
...
export default function TaskScopeManager({ initialScopes, onScopesChanged }: { initialScopes: TaskScope[], onScopesChanged: () => void }) {
  ...
  const refreshScopes = async () => {
     ...
     const updatedScopes = await getTaskScopes(orgId);
                    ...
                    <Card key={scope.id}>
                        <CardContent className="p-2">
                             <form onSubmit={(e) => {
                                 ...
                                 updateTaskScope(initialState, fd).then(refreshScopes);
                    ...
                    {scopes.map((scope) => (
                        ...
                        ? (
                            <TableRow key={scope.id}>
                                <TableCell colSpan={3}>
                                    <form onSubmit={(e) => { e.preventDefault(); updateTaskScope(initialState, new FormData(e.currentTarget)).then(refreshScopes); }} className="flex items-center gap-2">
